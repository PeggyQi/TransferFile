...
//      mContext.sendBroadcast(new Intent("DrawTickDone"));
    }
...
//      canvas.drawCircle(mCurrentPosition[0], mCurrentPosition[1], mSkipBallSize * mScale, mPaint);
  ...
  //获取圆心的x坐标
  int center =(int) getWidth() / 2+15;
  int center1 = (int) (center - getWidth() / 5)+5;
  //圆弧半径
  int radius = (int) (getWidth() / 2 - 50);
  public void drawTick(Canvas canvas)//绘制对勾
  {
    if(line1_x < radius / 3) {
      line1_x++;
      line1_y++;
    ...
    //画第二根线
    canvas.drawLine(center1 + line1_x , center + line1_y, center1 + line2_x, center + line2_y, mPaint);
